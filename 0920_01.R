...
#막대그래프(1. 그래프의 데이터 초기화, 2. 그래프 모양, 3. 제목과 라벨 설정정) 
...
#1분기, 2분기 데이터를 행으로 연결해서 하나의 데이터프레임에 저장
df = rbind(df1, df2)
df
...
#부서별 분기별 막대그래프 작성(조건1> 3분기 실적을 추가, 조건2> dsRdriver 패키지의 팔레트에 있는 색상을 막대에 설정, 
# 조건3>그래프 제목: 부서별 분기별 영업 실적, x축 제목: 부서명, y축 제목: 영업 매출(억원))
dept = c("영업1팀","영업2팀", "영업3팀")
sales = c(7, 10, 11)
...
#영업1팀의 6개월간 데이터프레임에 저장

month = c(1,2,3,4,5,6)
sales=c(3,3,5,5,7,4)
df1=data.frame(부서="영업1팀", 월=month, 매출=sales)

#영업2팀의 6개월간 데이터프레임에 저장
sales=c(2,2,4,8,9,6)
...
#점의 모양, 색상, 크기 설정
ggplot(data=women, aes(x=height,y=weight))+
  geom_point(shape=2, color='black', size=2)+
  labs(title = "미국 30-39세 여성의 키와 몸무게의 상관관계", x="키(inch)", y="체중(pound)")
